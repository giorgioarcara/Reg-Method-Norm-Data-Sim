...
dir.create("Results/Supplementary/Analysis5")

Capitani_vers = c("v1", "v2")

Arcara_vers = c("v1", "v2", "v3")
...
for (Capitani_ver in Capitani_vers){
  for (Arcara_ver in Arcara_vers){
    ...
    dep_num = c("TRAIN_MSE","TRAIN_SPECIFICITY",  "TRAIN_SENSITIVITY",  "TRAIN_PRECISION", "TRAIN_F1", "TRAIN_AGE_TRANSF_CORRESP", "TRAIN_EDU_TRANSF_CORRESP")
    ...
    write.csv(TRAIN_RES_UNIF,  row.names=F, file = paste("Results/Supplementary/Analysis1/TRAIN_RES_UNIF_A", Arcara_ver,"_C", Capitani_ver, ".csv", sep=""))
    write.csv(TRAIN_RES_COND,  row.names=F, file = paste("Results/Supplementary/Analysis1/TRAIN_RES_COND_A", Arcara_ver,"_C", Capitani_ver, ".csv", sep=""))
    write.csv(TEST_RES_UNIF,  row.names=F, file = paste("Results/Supplementary/Analysis1/TEST_RES_UNIF_A", Arcara_ver,"_C", Capitani_ver, ".csv", sep=""))
    write.csv(TEST_RES_COND,  row.names=F, file = paste("Results/Supplementary/Analysis1/TEST_RES_COND_A", Arcara_ver,"_C", Capitani_ver, ".csv", sep=""))
