...
# Launch the script STEP4a to actually generate the Figures from the objects that result as output from this step.
    ...
    for (sample_size in sample_size_values){
      ...
      for (noise_level in noise_values){
        ...
        for (sample_demo_fun in sample_demo_fun_values){
          ...
          res[c(iL, iL+1, iL+2), "n.sim"] = sim.params$n.sim
          res[c(iL, iL+1, iL+2), "n.subj.train"] = sim.params$n.subj.train
          res[c(iL, iL+1, iL+2), "n.subj.test"] = sim.params$n.subj.test
          res[c(iL, iL+1, iL+2), "P_score.sd"] = sim.params$P_score.sd
          res[c(iL, iL+1, iL+2), "sample.demo.fun"]=sample_demo_fun
          ...
          res[iL+2, "Method"] = "NULL"
          res[iL+2, "Version"] = "v1"
          res[iL+2, "TRAIN_MSE"] = mean(TRAIN_NULL_RESID_ERR)
          res[iL+2, "TRAIN_FP"] = mean(TRAIN_NULL_FP)
          res[iL+2, "TRAIN_FN"] = mean(TRAIN_NULL_FN)
          res[iL+2, "TRAIN_TP"] = mean(TRAIN_NULL_TP)
          res[iL+2, "TRAIN_TN"] = mean(TRAIN_NULL_TN)
          res[iL+2, "TRAIN_PRECISION"] = mean(TRAIN_NULL_PRECISION, na.rm=T)
          res[iL+2, "TRAIN_SENSITIVITY"] = mean(TRAIN_NULL_SENSITIVITY, na.rm=T)
          res[iL+2, "TRAIN_SPECIFICITY"] = mean(TRAIN_NULL_SPECIFICITY, na.rm=T)
          res[iL+2, "TRAIN_F1"] = mean(TRAIN_NULL_F1, na.rm=T)
          res[iL+2, "TEST_FP"] = mean(TEST_NULL_FP)
          res[iL+2, "TEST_FN"] = mean(TEST_NULL_FN)
          res[iL+2, "TEST_TP"] = mean(TEST_NULL_TP)
          res[iL+2, "TEST_TN"] = mean(TEST_NULL_TN)
          res[iL+2, "TEST_PRECISION"] = mean(TEST_NULL_PRECISION, na.rm=TRUE)
          res[iL+2, "TEST_SENSITIVITY"] = mean(TEST_NULL_SENSITIVITY, na.rm=TRUE)
          res[iL+2, "TEST_SPECIFICITY"] = mean(TEST_NULL_SPECIFICITY, na.rm=TRUE)
          res[iL+2, "TEST_F1"] = mean(TEST_NULL_F1, na.rm=TRUE)
          res[iL+2, "TEST_NBELOW"] = mean(TEST_NULL_NBELOW)
          
          res[iL+2, "TRAIN_PRECISION_NASUM"] = sum(is.na(TRAIN_NULL_PRECISION))
          res[iL+2, "TRAIN_SENSITIVITY_NASUM"] = sum(is.na(TRAIN_NULL_SENSITIVITY))
          res[iL+2, "TRAIN_SPECIFICITY_NASUM"] = sum(is.na(TRAIN_NULL_SPECIFICITY))
          res[iL+2, "TRAIN_F1_NASUM"] = sum(is.na(TRAIN_NULL_F1))
          res[iL+2, "TEST_PRECISION_NASUM"] = sum(is.na(TEST_NULL_PRECISION))
          res[iL+2, "TEST_SENSITIVITY_NASUM"] = sum(is.na(TEST_NULL_SENSITIVITY))
          res[iL+2, "TEST_SPECIFICITY_NASUM"] = sum(is.na(TEST_NULL_SPECIFICITY))
          res[iL+2, "TEST_F1_NASUM"] = sum(is.na(TEST_NULL_F1))
          
          res[iL+2, "TRAIN_AGE_TRANSF_CORRESP"]= 0 # there are no transformations with NULL model
          res[iL+2, "TRAIN_EDU_TRANSF_CORRESP"]= 0 # there are no transformations with NULL model
