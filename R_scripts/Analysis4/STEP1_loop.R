...
sample_demo_fun_values = c("sample.demo.cond") # tye distribution characteristics for demographic variables (see the functions in R functions)
Capitani_ver_values = c("v2") #the version of Capitani method (see supplementary materials of the article)
Arcara_ver_values = c("v3") # the version of the Arcara method (see supplementary materials of the article)

output_folder = "Analysis4/"
...
for (iLoop in n_start:length(loop_list)){
  ...
  source("R_scripts/Analysis4/STEP2_simulation.R", local=FALSE, verbose = F)
