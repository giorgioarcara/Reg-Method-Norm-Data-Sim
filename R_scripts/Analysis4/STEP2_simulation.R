...
TRAIN_NULL_TRANSF = list(NULL)
length(TRAIN_NULL_TRANSF)=n.sim

TRAIN_NULL_RS = NULL

TRAIN_GT_MODEL_TEXT = NULL
TRAIN_NULL_MODEL_TEXT = NULL

# object containing the formula of the models
TRAIN_NULL_FORMULA = list(NULL)
length(TRAIN_NULL_FORMULA)=n.sim

## TEST SET PERFORMANCE
TEST_NULL_NBELOW = NULL
TEST_NULL_RESID_ERR = NULL
TEST_NULL_FP = NULL 
TEST_NULL_FN = NULL
TEST_NULL_TP = NULL 
TEST_NULL_TN = NULL
...
for (iS in 1:n.sim){
  ...
  #######################
  # compute adj score with Capitani method
  NULL.res = lm(Obs_score~1, data=train.dat)
  
  TRAIN_NULL_TRANSF[[iS]] = NULL
  TRAIN_NULL_RS[iS]=summary(NULL.res)$r.squared
  TRAIN_NULL_FORMULA[[iS]]=formula(NULL.res)
  TRAIN_NULL_MODEL_TEXT[iS]=NULL
  
  # 2a) calculate Capitani's below cut-off obs
  Train_Adj.NULL = residuals(NULL.res) 
  Train_ES.NULL = ES(adjscores= residuals(NULL.res))$Adjusted_Scores
  Train_Below.NULL = residuals(NULL.res)<=Train_ES.NULL[1]
  Train_Above.NULL = residuals(NULL.res) > Train_ES.NULL[1]
  
  ###########################
  # COMPUTE TRAIN SET PERFORMANCE
  # RESID ERR (mean squared error)
  TRAIN_NULL_RESID_ERR[iS] = mean((Train_Adj.NULL-Train_Adj.GT)^2)
  # FALSE POSITIVES: values below for the method, but not below for the GT
  TRAIN_NULL_FP[iS] = length( setdiff(which(Train_Below.NULL), which(Train_Below.GT)) )
  # FALSE NEGATIVES: values below for the method, but not below for the GT
  TRAIN_NULL_FN[iS] = length(setdiff(which(Train_Above.NULL),  which(Train_Above.GT)))
  # TRUE POSITIVES: values below for the method and bewlow for the GT
  TRAIN_NULL_TP[iS] = length( intersect(which(Train_Below.NULL), which(Train_Below.GT)) )
  # TRUE NEGATIVES: values below for the method, but not below for the GT
  TRAIN_NULL_TN[iS] = length(intersect(which(Train_Above.NULL),  which(Train_Above.GT)))
  
  ### TEST SET PERFORMANCE
  NULL.test_resid = test.dat$Obs_score - predict(NULL.res)[1] # get only one elements cause prediction is always the same
  
  TEST_NULL_RESID_ERR[iS] = mean((NULL.test_resid - test.dat$P_score)^2)
  
  Test_Below.NULL = NULL.test_resid <= Train_ES.NULL[1]
  Test_Above.NULL = NULL.test_resid > Train_ES.NULL[1]
  
  ## COMPUTE TEST SET PERFORMANCE
  TEST_NULL_NBELOW[iS] = sum ( Test_Below.NULL )
  
  # FALSE POSITIVES: values below for the method, but not below for the GT
  TEST_NULL_FP[iS] = length( setdiff(which(Test_Below.NULL), which(Test_Below.GT)) )
  # FALSE NEGATIVES: values below for the method, but not below for the GT
  TEST_NULL_FN[iS] = length(setdiff(which(Test_Above.NULL),  which(Test_Above.GT)))
  # TRUE POSITIVES: values below for the method and bewlow for the GT
  TEST_NULL_TP[iS] = length( intersect(which(Test_Below.NULL), which(Test_Below.GT)) )
  # TRUE NEGATIVES: values below for the method, but not below for the GT
  TEST_NULL_TN[iS] = length(intersect(which(Test_Above.NULL),  which(Test_Above.GT)))
...
## NULL - PERFORMANCE
# TRAIN
TRAIN_NULL_SENSITIVITY =  TRAIN_NULL_TP/(TRAIN_NULL_TP+TRAIN_NULL_FN)
TRAIN_NULL_PRECISION = TRAIN_NULL_TP/(TRAIN_NULL_TP+TRAIN_NULL_FP)
TRAIN_NULL_SPECIFICITY = TRAIN_NULL_TN/(TRAIN_NULL_TN+TRAIN_NULL_FP)
TRAIN_NULL_F1 = 2*(TRAIN_NULL_SENSITIVITY*TRAIN_NULL_PRECISION)/(TRAIN_NULL_SENSITIVITY+TRAIN_NULL_PRECISION)
### TEST 
TEST_NULL_SENSITIVITY =  TEST_NULL_TP/(TEST_NULL_TP+TEST_NULL_FN)
TEST_NULL_PRECISION = TEST_NULL_TP/(TEST_NULL_TP+TEST_NULL_FP)
TEST_NULL_SPECIFICITY = TEST_NULL_TN/(TEST_NULL_TN+TEST_NULL_FP)
TEST_NULL_F1 = 2*(TEST_NULL_SENSITIVITY*TEST_NULL_PRECISION)/(TEST_NULL_SENSITIVITY+TEST_NULL_PRECISION)
...
save(sim.params, adjscores_A2024,
     ...
     TEST_ARC_NBELOW, 
     TRAIN_NULL_RS,TRAIN_NULL_FORMULA, TRAIN_GT_MODEL_TEXT, TRAIN_NULL_RESID_ERR,  TRAIN_NULL_TRANSF, TRAIN_NULL_MODEL_TEXT,
     TRAIN_NULL_FN, TRAIN_NULL_FP,TRAIN_NULL_TP, TRAIN_NULL_TN, TRAIN_NULL_PRECISION, TRAIN_NULL_SENSITIVITY, TRAIN_NULL_SPECIFICITY, TRAIN_NULL_F1,
     TEST_NULL_FN, TEST_NULL_FP,TEST_NULL_TP, TEST_NULL_TN, TEST_NULL_PRECISION, TEST_NULL_SENSITIVITY, TEST_NULL_SPECIFICITY, TEST_NULL_F1,     
     TEST_NULL_NBELOW, 
     file=file_name)
