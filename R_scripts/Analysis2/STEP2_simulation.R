...
# The datasets from which the empirical distribution of data is generated is set in the section GET EMPIRICAL DISTRIBUTION OF DEMOGRAPHIC DATA, below.
...
#sample.demo.fun = "sample.demo.cond"
...
## SET OUTPUT NAME
file_name = paste("Results/", output_folder, "Sim_Nsize_", n.subj.train, "_Noise_", P_score.sd,"_A", Arcara_ver, "_C", Capitani_ver, "_", sample_demo_val, tag, ".RData", sep="")
print(file_name)

### GET EMPIRICAL DISTRIBUTION OF DEMOGRAPHIC DATA
demo.dat = read.csv(paste("Original_Data/", sample_demo_val,"_Dataset.csv", sep=""))

demo.dat$Sex = as.numeric(demo.dat$Sex=="F")
...
# fix value with 0 (replacing with 1)
demo.dat[demo.dat$edu_values_o==0, "edu_values_o"] = 1
...
for (iS in 1:n.sim){
  ...
  train.demo.data = demo.dat[sample(1:dim(demo.dat)[1], n.subj.train, replace=T), ]
  ...
  test.demo.data = demo.dat[sample(1:dim(demo.dat)[1], n.subj.test, replace=T), ]
