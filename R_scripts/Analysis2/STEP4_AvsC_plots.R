# This script generate all plots from the objects created in STEP3.
...
input_folder="Analysis2/"

dir.create(paste("Figures/", input_folder, sep=""))

sample_demo_values = c("GEMS", "MOCA", "TELEGEMS")
  ...
  for (sample_demo_val in sample_demo_values){
    for (Arcara_ver in Arcara_ver_values){
      for (Capitani_ver in Capitani_ver_values){
        ...
        #sample_demo_val = "sample.demo.cond"
        curr_res = res[res$sample.demo.val == sample_demo_val, ]
        ...
        jpeg(file=paste("Figures/", input_folder, "Performance_A", Arcara_ver,"_C", Capitani_ver, "_", sample_demo_val, "_", dep, ".jpeg", sep=""), width=1800, height=800, res = 200)
        ...
        text(0, 0.80, labels=paste("Arcara: ", Arcara_ver, " - Capitani: ", Capitani_ver, " ", sample_demo_val, sep=""), cex=1, font=1)
