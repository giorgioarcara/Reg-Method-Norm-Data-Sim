...
for (iCAP in 1:length(Capitani_ver_values)){
  ...
  for (iARC in 1:length(Arcara_ver_values)){
    ...
    sample_demo_values = c("MOCA", "GEMS", "TELEGEMS")
    tag=""
    
    ## Create empty results
    res_size = length(sample_size_values)*length(noise_values)*length(sample_demo_values)
    ...
    for (sample_size in sample_size_values){
      ...
      for (noise_level in noise_values){
        ...
        for (sample_demo_val in sample_demo_values){
          ...
          load(paste("Results/", input_folder, "Sim_Nsize_", n.subj.train, "_Noise_", P_score.sd,"_A", Arcara_ver, "_C", Capitani_ver, "_", sample_demo_val, tag, ".RData", sep=""))
          ...
          res[c(iL, iL+1), "sample.demo.val"]=sample_demo_val
