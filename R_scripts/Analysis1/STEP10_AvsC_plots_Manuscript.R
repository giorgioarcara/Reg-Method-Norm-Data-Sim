...
output_folder="Manuscript/"

dir.create(paste("Figures/", output_folder, sep=""))
...
dep_labels = c("Normative Data - MSE", "Normative Data - Precision", "Normative Data - Sensitivity", "Normative Data - Specificity", "Normative Data - F1", 
                 "New Participants - Precision", "New Participants - Sensitivity", "New Participants - Specificity", "New Participants - F1")
## select variables

for (iDep in 1:length(deps)){
  dep = deps[iDep]
  dep_label = dep_labels[iDep]
  ...
  for (sample_demo_fun in sample_demo_fun_values){
    for (Arcara_ver in Arcara_ver_values){
      for (Capitani_ver in Capitani_ver_values){
        ...
        jpeg(file=paste("Figures/", output_folder, "Performance_A", Arcara_ver,"_C", Capitani_ver, "_", sample_demo_fun, "_", dep, ".jpeg", sep=""), width=1800, height=800, res = 200)
        par(mfrow=c(1,3), mar=c(5.1, 4.1, 4.1+3, 2.1))
        curr_noise = 0.5
        pt_size = 1
        
        plot(NA, xlim = c(xrange), ylim=yrange, ylab=dep_label, xlab="Sample Size", main="")
        ...
        text(0, 0.95, labels=dep_label, cex=1.5, font=2) # remove underscore from dep names
        #text(0, 0.80, labels=paste("Arcara: ", Arcara_ver, " - Capitani: ", Capitani_ver, " ", sample_demo_fun, sep=""), cex=1, font=1) # not plotting version and norm data distribution
