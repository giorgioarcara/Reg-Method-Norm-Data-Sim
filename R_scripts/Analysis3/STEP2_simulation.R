...
# The data of the test set are systematically modified in the section SET LOWER EXPECTED SCORE, below.
...
for (iS in 1:n.sim){
  ...
  ############################################################################
  ############################################################################
  ### SET LOWER EXPECTED SCORE
  # worsen systematically the performance of half of participants
  prop = 3 # 1/prop people will be the patients
  patients = sample(1:dim(test.dat)[1], round(dim(test.dat)[1]/3))
  test.dat[patients, "P_score" ] = Train_ES.GT[1] - abs(rnorm(length(patients), mean = 0, sd=0.3))
  test.dat[patients, "Obs_score" ] =  test.dat[patients, "Demo_score" ] + test.dat[patients, "P_score" ]

   #hist(test.dat$Obs_score)
